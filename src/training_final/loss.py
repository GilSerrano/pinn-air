...
def position_error(y_hat, y, target_time, decay_factor=0.1):
    
    # Compute the exponential decay
    exp_decay = 1 * torch.exp(-decay_factor*torch.arange(0, target_time, 1)).to("cuda")
    
    # Compute the discounted position error
    return torch.sum((torch.norm(y_hat[:, :, 0:3] - y[:, :, 0:3], dim=2) ** 2) * exp_decay)

def position_error_payload(y_hat, y, target_time, decay_factor=0.1):
...
def velocity_error(y_hat, y, target_time, decay_factor=0.1):
...
def quaternion_error(y_hat, y, target_time, decay_factor=0.1):
...
def physics_error(y_hat, x, y, target_time, physics_model, decay_factor=0.1):
    ...
    pos_error = position_error(y_hat, physics_pred, target_time, decay_factor=decay_factor)
    vel_error = velocity_error(y_hat, physics_pred, target_time, decay_factor=decay_factor)
    quat_error = quaternion_error(y_hat, physics_pred, target_time, decay_factor=decay_factor)
