...
class Plot:
    ...
    def __init__(self, x, y, y_hat, time, time2):
        self.x = x
        self.y = y
        self.y_hat = y_hat
        self.time = time
        self.time2 = time2
        self.fig = plt.figure(figsize=(4, 2), dpi=300)
    ...
    def draw(self, **kwargs):
        ...
        plt.plot(self.time, torch.cat((self.x[:, 1], self.y[:, idx[0]]), dim=0).numpy(), color=(0,1,0,0.7))
        plt.plot(self.time, torch.cat((self.x[:, 2], self.y[:, idx[0]]), dim=0).numpy(), color=(0,0,1,0.5))
...
def main():
    ...
    pos_plot = Plot(x[..., 0:3], y[..., 0:3], y_hat[..., 0:3], time, time2)
    vel_plot = Plot(x[..., 3:6], y[..., 3:6], y_hat[..., 3:6], time, time2)
    qtr_plot = Plot(x[..., 6:10], y[..., 6:10], y_hat[..., 6:10], time, time2)
    ld_plot  = Plot(x[..., 10:13], y[..., 10:13], y_hat[..., 10:13], time, time2)
