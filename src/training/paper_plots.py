...
from main import model, load_best_model, target_time
...
with torch.no_grad():
    ...
    plt.plot(time2, torch.cat((x_last, y_hat[0, :, 0]), dim=0).numpy(), linestyle="--", color="red", label="$p_x$")        # x
    plt.plot(time2, torch.cat((y_last, y_hat[0, :, 1]), dim=0).numpy(), linestyle="--", color="green", label="$p_y$")        # y
    plt.plot(time2, torch.cat((z_last, y_hat[0, :, 2]), dim=0).numpy(), linestyle="--", color="blue", label="$p_z$")        # z

    # Plot the real position
    plt.plot(time, torch.cat((x[:, 0], y[:, 0]), dim=0).numpy(), color=(1,0,0,0.6))        # x
    plt.plot(time, torch.cat((x[:, 1], y[:, 1]), dim=0).numpy(), color=(0,1,0,0.7))        # y
    plt.plot(time, torch.cat((x[:, 2], y[:, 2]), dim=0).numpy(), color=(0,0,1,0.5))        # z
    ...
    plt.plot(time2, torch.cat((x_last, y_hat[0, :, 3]), dim=0).numpy(), linestyle="--", color="red", label="$v_x$")        # x
    plt.plot(time2, torch.cat((y_last, y_hat[0, :, 4]), dim=0).numpy(), linestyle="--", color="green", label="$v_y$")        # y
    plt.plot(time2, torch.cat((z_last, y_hat[0, :, 5]), dim=0).numpy(), linestyle="--", color="blue", label="$v_z$")        # z

    # Plot the real position
    plt.plot(time, torch.cat((x[:, 3], y[:, 3]), dim=0).numpy(), color=(1,0,0,0.6))        # x
    plt.plot(time, torch.cat((x[:, 4], y[:, 4]), dim=0).numpy(), color=(0,1,0,0.7))        # y
    plt.plot(time, torch.cat((x[:, 5], y[:, 5]), dim=0).numpy(), color=(0,0,1,0.5))        # z
    ...
    plt.plot(time2, torch.cat((w_last, -y_hat[0, :, 6]), dim=0).numpy(), linestyle="--", color="orange", label="$q_w$")        # w
    plt.plot(time2, torch.cat((x_last, -y_hat[0, :, 7]), dim=0).numpy(), linestyle="--", color="red", label="$q_x$")        # x
    plt.plot(time2, torch.cat((y_last, -y_hat[0, :, 8]), dim=0).numpy(), linestyle="--", color="green", label="$q_y$")        # y
    plt.plot(time2, torch.cat((z_last, -y_hat[0, :, 9]), dim=0).numpy(), linestyle="--", color="blue", label="$q_z$")        # z

    # Plot the real position
    plt.plot(time, torch.cat((x[:, 6], y[:, 6]), dim=0).numpy(), color=(1, 220/255, 0.0,1.0))        # w
    plt.plot(time, torch.cat((x[:, 7], y[:, 7]), dim=0).numpy(), color=(1,0,0,0.6))                  # x
    plt.plot(time, torch.cat((x[:, 8], y[:, 8]), dim=0).numpy(), color=(0,1,0,0.7))                  # y
    plt.plot(time, torch.cat((x[:, 9], y[:, 9]), dim=0).numpy(), color=(0,0,1,0.5))                  # z
    plt.grid()
    ...
    plt.plot(time2, torch.cat((x_last, y_hat[0, :, 10]), dim=0).numpy(), linestyle="--", color="red", label="$p^L_x$")          # x
    plt.plot(time2, torch.cat((y_last, y_hat[0, :, 11]), dim=0).numpy(), linestyle="--", color="green", label="$p^L_y$")        # y
    plt.plot(time2, torch.cat((z_last, y_hat[0, :, 12]), dim=0).numpy(), linestyle="--", color="blue", label="$p^L_z$")         # z

    # Plot the real position
    plt.plot(time, torch.cat((x[:, 10], y[:, 10]), dim=0).numpy(), color=(1,0,0,0.6))        # x
    plt.plot(time, torch.cat((x[:, 11], y[:, 11]), dim=0).numpy(), color=(0,1,0,0.7))        # y
    plt.plot(time, torch.cat((x[:, 12], y[:, 12]), dim=0).numpy(), color=(0,0,1,0.5))        # z
    plt.grid()
