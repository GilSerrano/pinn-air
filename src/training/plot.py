...
best_model = torch.load("output4/best_model_idx.pth.tar")
best_model_idx = best_model["best_model_idx"]

load_best_model(best_model_idx, save_dir="output4/")
...
#     plt.figure()
#     plt.plot(time, torch.cat((x[:, 6], y[:, 6]), dim=0).numpy(force=True), label="x")        # qx
#     plt.plot(time, torch.cat((x[:, 7], y[:, 7]), dim=0).numpy(force=True), label="y")        # qy
#     plt.plot(time, torch.cat((x[:, 8], y[:, 8]), dim=0).numpy(force=True), label="z")        # qz
#     plt.plot(time, torch.cat((x[:, 9], y[:, 9]), dim=0).numpy(force=True), label="z")        # qw

#     time2 = torch.arange(x.shape[0], x.shape[0] + y_hat.shape[1]).numpy(force=True)
#     plt.plot(time2, y_hat[0, :, 6].numpy(force=True), label="x_hat")        # qx
#     plt.plot(time2, y_hat[0, :, 7].numpy(force=True), label="y_hat")        # qy
#     plt.plot(time2, y_hat[0, :, 8].numpy(force=True), label="z_hat")        # qz
#     plt.plot(time2, y_hat[0, :, 9].numpy(force=True), label="z_hat")        # qw
...
#     time2 = torch.arange(x.shape[0], x.shape[0] + y_hat.shape[1]).numpy(force=True)
#     plt.plot(time2, y_hat[0, :, 10].numpy(force=True), label="x_hat")        # x
#     plt.plot(time2, y_hat[0, :, 11].numpy(force=True), label="y_hat")        # y
#     plt.plot(time2, y_hat[0, :, 12].numpy(force=True), label="z_hat")        # z
